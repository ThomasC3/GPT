...
public protocol SideViewManagerDelegate: AnyObject {
...
public class SideViewManager {
    ...
    /// The view which will have its view moved
    ...
    /// Determines whether the root view should shift to accomodate the changes in the side view position
    public var isRootViewAdaptive = false
    ...
    public lazy var dismissGesture: UITapGestureRecognizer = {
        let gesture = UITapGestureRecognizer(target: self, action: #selector(dismissHandler))
    ...
    var currentOffset: CGFloat {
        ...
        case .horizontal where isOppositeDirection:
            return (-offOrigin.x - sideOrigin.x) / (onOrigin.x - offOrigin.x)
        case .horizontal:
            return (offOrigin.x - sideOrigin.x) / (offOrigin.x - onOrigin.x)
        case .vertical where isOppositeDirection:
            return (-offOrigin.y - sideOrigin.y) / (onOrigin.y - offOrigin.y)
        case .vertical:
    ...
    public init(controller: UIViewController, startingFrame: CGRect, endingFrame: CGRect) {
    ...
    public convenience init(controller: UIViewController) {
        let windowSize = UIApplication.shared.keyWindow?.frame.size ?? .zero
        let size = CGSize(width: windowSize.width - 100, height: windowSize.height)
        
        let startingFrame = CGRect(origin: CGPoint(x: -size.width, y: 0), size: size)
        let endingFrame = CGRect(origin: .zero, size: size)

//        let startingFrame = CGRect(origin: CGPoint(x: windowSize.width, y: 0), size: size)
//        let endingFrame = CGRect(origin: CGPoint(x: windowSize.width - size.width, y: 0), size: size)

        self.init(controller: controller, startingFrame: startingFrame, endingFrame: endingFrame)
    }
    ...
    public func move(to offset: CGFloat, duration: TimeInterval = 0.25) {
        let clampedOffset = offset.clamped(to: 0...1)
        let newOffset = isOppositeDirection ? 1 - clampedOffset : clampedOffset
        let newFrame = CGRect(offset: newOffset, starting: startingFrame, ending: endingFrame)
        UIView.animate(withDuration: duration, delay: 0, options: .curveEaseOut, animations: {
            self.view.frame = newFrame

            if self.isRootViewAdaptive, let root = UIApplication.shared.activeWindow?.rootViewController?.view {
                let startingOrigin = self.startingFrame.origin
                let endingOrigin = self.endingFrame.origin
                let xOffset = (startingOrigin.x - newFrame.origin.x) * (startingOrigin.x < endingOrigin.x ? -1 : 1)
                let yOffset = (startingOrigin.y - newFrame.origin.y) * (startingOrigin.y < endingOrigin.y ? -1 : 1)
                root.frame.origin = CGPoint(x: xOffset, y: yOffset)
            }
        }) { _ in
    ...
    @objc private func dismissHandler(_ recognizer: UITapGestureRecognizer) {
    ...
    @objc private func panHandler(_ recognizer: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            let panTranslation = translation.clamped(to: (maxOffset - comparisonOffset)...maxOffset)
