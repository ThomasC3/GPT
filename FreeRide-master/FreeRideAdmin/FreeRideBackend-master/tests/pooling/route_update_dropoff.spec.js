...
describe('Route update', () => {
  before(async () => {
    ...
    const driverInfo = {
      currentLocation: {
        coordinates: [keyLoc.a1[1], keyLoc.a1[0]],
        type: 'Point'
      },
  ...
  describe('Route update for early dropoff', () => {
    it('Should assign driver 1 to ride request 1 and pick up', async () => {
      // Request 1 created
      logger.debug('REQUEST 1 CREATED');
      logger.debug([keyLoc.a1[2], keyLoc.a1[0], keyLoc.a1[1]]);
      logger.debug([keyLoc.b0[2], keyLoc.b0[0], keyLoc.b0[1]]);
      ...
      await request(app)
        ...
        .send({
          ...
          origin: {
            address: keyLoc.a1[2],
            latitude: keyLoc.a1[0],
            longitude: keyLoc.a1[1]
          },
          message: null,
          location: location._id.toString(),
          destination: {
            address: keyLoc.b0[2],
            latitude: keyLoc.b0[0],
            longitude: keyLoc.b0[1]
          }
        })
        .expect(200)
        .end();
    ...
    it('Should assign driver 1 to ride request 2 and pick up', async () => {
      ...
      await request(app)
        ...
        .send({
          ...
          destination: {
            address: keyLoc.b1[2],
            latitude: keyLoc.b1[0],
            longitude: keyLoc.b1[1]
          }
        })
        .expect(200)
        .end();
    ...
    it('Should assign driver 1 to ride request 3 and dropoff request 3', async () => {
      // Request 1 created
      logger.debug('REQUEST 3 CREATED');
      logger.debug([keyLoc.a1[2], keyLoc.a1[0], keyLoc.a1[1]]);
      logger.debug([keyLoc.b3[2], keyLoc.b3[0], keyLoc.b3[1]]);
      ...
      await request(app)
        ...
        .set('Accept', 'application/json')
        .set('Content-Type', 'application/json')
        .set('Authorization', `Bearer ${rider3Token}`)
        .send({
          passengers: 1,
          isADA: false,
          origin: {
          ...
          destination: {
            address: keyLoc.b3[2],
            latitude: keyLoc.b3[0],
            longitude: keyLoc.b3[1]
          }
        })
        .expect(200)
        .end();
      ...
      await dropOff(driverToken, ride3, app, request, domain);

      route1 = await Routes.findOne({ driver });
      sinon.assert.match(route1.stops[route1.stops.length - 4].ride, ride3._id);
      return sinon.assert.match(route1.stops[route1.stops.length - 4].status, 'done');
    });
  });
});
