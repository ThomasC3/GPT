...
describe('Service matching pooling', () => {
  before(async () => {
      ...
      drivers.push(await createDriverLogin(
        {
          ...
          isADA: i < 2,
          vehicle: await createGEMVehicle(i < 2, location._id, { driverDump: true })
        },
  ...
  describe('Create 3 non-ADA requests', () => {
    ...
    it('Should route 3 non-ADA requests to mixed_service and passenger_only drivers', async () => {
      ...
      for (let i = 0; i < 3; i += 1) {
        if (i === 2) {
          // Add mixed_service driver
          await Drivers.updateOne({ _id: drivers[1].driver._id }, { $set: { isAvailable: true } });
    ...
    it('Should route 3 non-ADA requests to the passenger_only driver', async () => {
      // Remove mixed_service driver
      await Drivers.updateOne({ _id: drivers[1].driver._id }, { $set: { isAvailable: false } });
  ...
  describe('Create 3 ADA requests', () => {
    ...
    it('Should route 3 non-ADA requests to the ada_only driver', async () => {
      // Remove mixed_service driver
      await Drivers.updateOne({ _id: drivers[1].driver._id }, { $set: { isAvailable: false } });
